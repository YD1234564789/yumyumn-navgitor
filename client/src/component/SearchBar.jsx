...
export default function SearchBar(){  
  const { userlocation, searchr, setSearchr } = useContext(InformContext)
  ...
  return(
    <form id="filterForm" onSubmit="event.preventDefault(); searchPlaces();">
            
      <label htmlFor="type">餐廳類別：</label>
      <select id="type" name="type" className="me-3">
      ...
      <label htmlFor="distance">選擇搜索距離：</label>
      <select id="distance" name="distance" className="me-3">
      ...
      <select id="rating" name="rating" className="me-3">
      ...
      <label>價格等級：</label>
      <MyComponent />
      <input type="hidden" id="latitude" name="latitude" value={userlocation.lat} />
      <input type="hidden" id="longitude" name="longitude" value={userlocation.lng} />
      <button className="btn btn-primary btn-sm" type="submit">搜尋餐廳</button>
    </form>
  )
}